  ...
  const rankedUsers = useMemo(() => {
    return users
      .map(user => ({
        ...
        isWinner: user.total_submissions >= 15
      }))
  ...
  const LeaderboardTable = ({ users, showSelection = false }: { users: any[], showSelection?: boolean }) => (
    <div className="overflow-x-auto">
      <Table>
        ...
        <TableBody>
          {users.map((user) => (
            <TableRow key={user.id} className={user.rank <= 3 ? "bg-gradient-to-r from-primary/5 to-success/5" : ""}>
              {showSelection && (
                <TableCell>
                  <Switch
              ...
              <TableCell>
                <div className="space-y-1">
                  <div className="text-sm">{user.total_submissions}/15 days</div>
                  <Progress value={(user.total_submissions / 15) * 100} className="h-2 w-20" />
                </div>
              </TableCell>
              <TableCell>
                {getStatusBadge(user)}
        ...
        <TabsContent value="winners">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Users who completed all 15 days of the challenge
              </CardDescription>
            </CardHeader>
            <CardContent>
              {winners.length === 0 ? (
